   ...
   try {
        ...
        if(con!=null) {
          // out.println("Successfully connected to " + "MySQL server using TCP/IP..." + "<br>");
		   ...
		   sql="select * from tbl_booking where cid="+Integer.parseInt(uid)+"";
  ...
  %>
                <tr>
				  ...
				  <td><%   out.print(status); %></td>
                    ...
                    <td><button name="btnsubmit" class="feedback-btn" data-booking-id="<% out.print(bookid); %>" onclick="openFeedbackForm(this)" value="<% out.print(bookid); %>">Give Feedback</button></td>
                    <td><button class="download-receipt-btn" onclick="">Download Receipt</button></td>
                </tr>
               <%
			      }
	...
	<form name="frmfeed" action="savefeedback.jsp" method="post">
	<%
	session.setAttribute("bookid",String.valueOf(bookid));
	%>
        <h3>Submit Feedback for Booking ID <span id="bookingId">#</span></h3>
        
        <textarea placeholder="Enter your feedback here..." name="txtfeedback"></textarea>
        <button value="Submit" name="Submit">Submit Feedback</button>
		</form>
    </div>
